...
int64 nChainStartTime = 1406463863;
...
const string strMessageMagic = "prismacoin Signed Message:\n";
...
int64 GetProofOfStakeReward(int64 nCoinAge, unsigned int nBits, unsigned int nTime, int nHeight)
{
    ...
    nRewardCoinYear = MAX_BCH_PROOF_OF_STAKE;
...
static const int64 nTargetTimespan = 60 * 10; // 30 blocks  
...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex, bool fJustCheck)
{
    ...
    bool fEnforceBIP30 = true; // Always active in prismacoin
    bool fStrictPayToScriptHash = true; // Always active in prismacoin
...
bool CheckDiskSpace(uint64 nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "prismacoin", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    if (fTestNet)
    {
        ...
        nStakeMaxAge = 2 * 30 * 60; // test net min age is 30 min
    ...
    if (mapBlockIndex.empty())
    {
        ...
        // Genesis block
        const char* pszTimestamp = "Do what you can do!";
        CTransaction txNew;
        ...
        assert(block.hashMerkleRoot == uint256("0xca0c8db7ee67761f9a846ac60569caa2306903af9c4e8fbecfa03c933634abab"));
...
unsigned char pchMessageStart[4] = { 0x7B, 0x35, 0x22, 0x05 };
        ...
        if (fProofOfStake)
        {
            ...
            if (pblock->IsProofOfStake())
            {
                if (!pblock->SignBlock(*pwalletMain))
                    continue;
                printf("CPUMiner : Holy Shit! You found a proof-of-stake block, get yerself a hooker! %s\n", pblock->GetHash().ToString().c_str()); 
